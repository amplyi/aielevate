/**
 * AI Elevate Decision Room Â gated session simulator (API proxy, no client API key).
 */
(function () {
 ...
 const ARCHETYPES = {
 corporate: 'A multinational organisation under regulatory pressure facing a critical strategic launch.',
 ...
 growth: 'A scaling organisation at the edge of breakthrough or structural failure.',
 };
 ...
 if (i18n && i18n.getLocale() === 'nl') return i18n.getPack('decisionRoomUi') || {};
 return {};
 }
 ...
 'This is an EDMP (Enterprise Decision Memory Platform) decision pressure simulation Â not a game.'
 );
 }
 ...
 $('drSetupError').textContent = drUi().industryRoleRequired || tr('Industry and role are required.');
 return;
 }
 ...
 updateMetrics();
 $('drLogList').innerHTML = `<div class="dr-muted-pad">${drUi().awaitingFirst || tr('Awaiting first decision..')}</div>`;
 ...
 <div class="dr-intro-sub">${tr('Experience how decisions form under pressure, and what your organisation would fail to reconstruct later. Select a scenario archetype.')}</div>
 <div class="dr-scenario-grid">
 <button type="button" class="dr-scenario-btn" data-arch="corporate"><span class="dr-sc-tag">CORPORATE</span>${tr('Regulatory pressure and strategic launch under scrutiny')}</button>
 <button type="button" class="dr-scenario-btn" data-arch="government"><span class="dr-sc-tag">PUBLIC SECTOR</span>${tr('Fast-moving crisis with governance visibility')}</button>
 <button type="button" class="dr-scenario-btn" data-arch="regulated"><span class="dr-sc-tag">REGULATED</span>${tr('Audit, compliance, and accountability tension')}</button>
 <button type="button" class="dr-scenario-btn" data-arch="growth"><span class="dr-sc-tag">GROWTH</span>${tr('Scaling organisation at breakthrough or collapse')}</button>
 </div>`;
 ...
 'Generate the outcome in 2 to 3 sentences. Hint at traceability, accountability, and memory gaps. Output text only.';
 ...
 '{\n "verdict": "2-word label",\n "summary": "3 to 4 sentences on decision performance and traceability gaps",\n' +
 ' "scores": {"strategic_clarity":0,"decision_traceability":0,"risk_tolerance":0,"ethical_resolve":0,"decisiveness":0,"adaptability":0},\n' +
 ' "edmp_gaps": "2 to 3 sentences on what reasoning would be lost if key people left",\n' +
 ' "psychological_profile": "2 sentences",\n "pressure_patterns": "2 sentences",\n' +
 ' "critical_reasoning_coaching": "3 to 4 sentences referencing actual choices",\n' +
 ...
 <div style="font-family:ui-monospace,monospace;font-size:16px;letter-spacing:0.12em;color:${failed ? 'var(--dr-danger)' : 'var(--dr-safe)'}">${failed ? (drUi().criticalFailure || tr('CRITICAL FAILURE')) : (drUi().sessionComplete || tr('SESSION COMPLETE'))}</div>
 ...
 <span style="color:var(--dr-safe)">INT ${state.metrics.integrity}</span> Â
 <span style="color:var(--dr-accent)">VEL ${state.metrics.velocity}</span> Â
 ...
 ${data?.edmp_gaps ? `<p style="color:var(--dr-text);line-height:1.7;margin-bottom:16px"><strong style="color:var(--dr-accent)">${drUi().decisionMemoryGaps || tr('Decision memory gaps:')}</strong> ${data.edmp_gaps}</p>` : ''}
 ...
 ${data?.recommended_next_step ? `<p style="color:var(--dr-accent2);margin-bottom:16px">${drUi().recommended || tr('Recommended:')} ${data.recommended_next_step}</p>` : ''}
 <div style="text-align:center;margin-top:16px;display:flex;gap:10px;justify-content:center;flex-wrap:wrap">
 <button type="button" class="dr-restart-btn" id="drRestartBtn">${drUi().newSession || tr('NEW SESSION')}</button>
 <button type="button" class="dr-btn dr-btn-primary" id="drEngageBtn">${drUi().viewPrograms || tr('VIEW PROGRAMS')}</button>
 </div>`;
 if (!data && rawFallback) {
 ...
 window.DecisionRoom = { onViewActive, readTokenFromUrl, refreshLocale };

 function refreshLocale() {
 if (window.AIE_I18N && typeof window.AIE_I18N.applyPhraseMap === 'function') {
 window.AIE_I18N.applyPhraseMap(document.getElementById('decision-room'));
 }
 const token = $('drTokenInput');
 if (token) {
 token.setAttribute('placeholder', drUi().tokenPlaceholder || tr('Paste token from your confirmation email'));
 }
 }
