/**
 * EDMP Digital Assessment Engine ù preview (public) + full diagnostic (token-gated).
 */
(function () {
  ...
  function previewCfg() {
    return window.EDMP_ASSESSMENT_PREVIEW || { dimensions: [], sampleReport: {}, sampleRecommendations: [] };
  }
  ...
  let state = {
    meta: { organization: '', contact: '', role: '' },
    answers: {},
    step: 0,
    report: null,
    consultantNotes: '',
  ...
  function resetState() {
    state = { meta: { organization: '', contact: '', role: '' }, answers: {}, step: 0, report: null, consultantNotes: '' };
  ...
  async function validateAndLoadConfig() {
    ...
    try {
      ...
      const org = validateData.session?.organization;
      if (org && !state.meta.organization) state.meta.organization = org;
      return true;
    } catch (_) {
      return false;
    }
  ...
  function computeReport() {
    ...
    const org = state.meta.organization || 'The organization';
    const execSummary = (
  ...
  function renderPreview() {
    ...
    const dimCards = dims.map((d) => `
      <article class="edmp-preview-dim glass">
        ...
        <p class="edmp-preview-note">Full diagnostic: 5 scored questions in this dimension (paid session).</p>
      </article>
    `).join('');
    ...
    mount.innerHTML = `
      <div class="edmp-assess-card edmp-preview-card">
        ...
        <div class="edmp-preview-gate glass">
          <h3>Full assessment access</h3>
          <p>The complete 30-question diagnostic, scoring engine, tailored recommendations, and PDF report require a paid EDMP Readiness Assessment session.</p>
          <label class="edmp-assess-field">
            <span>Session access token</span>
            <div class="edmp-token-row">
              <input id="edmpTokenInput" type="text" placeholder="Paste token from your confirmation email" autocomplete="off" />
              <button type="button" class="edmp-assess-btn edmp-assess-btn-primary" id="edmpTokenBtn">Enter</button>
            </div>
          </label>
          <p id="edmpGateError" class="edmp-assess-error" role="alert"></p>
          <div class="edmp-assess-actions">
            <button type="button" class="edmp-assess-btn" id="edmpRequestAssessment" data-engage-product="assessment" data-source="assessment-preview">Request Assessment</button>
          </div>
        </div>

        <section class="edmp-preview-section">
          <h3>Six EDMP dimensions</h3>
        ...
        <section class="edmp-preview-section edmp-preview-sample-report">
          ...
          <h3>Sample report output</h3>
          <p class="edmp-preview-disclaimer">Illustrative scores only. Not generated from your organization.</p>
          <div class="edmp-report-hero edmp-preview-hero">
            <div>
              <strong>${escapeHtml(sample.organization || 'Example')}</strong>
              <p class="edmp-assess-lead">Board-ready EDMP Readiness Assessment Report</p>
            </div>
        ...
        <section class="edmp-preview-section">
          ...
          <p class="edmp-preview-disclaimer">Examples of recommendation style. Full engine maps rules to your scores after payment.</p>
    ...
    $('edmpTokenBtn')?.addEventListener('click', async () => {
      ...
      if (!sessionToken) {
        if (errEl) errEl.textContent = 'Enter your session token.';
        return;
      }
      sessionStorage.setItem(TOKEN_KEY, sessionToken);
      if (errEl) errEl.textContent = 'Validating session...';
      const ok = await validateAndLoadConfig();
      if (!ok) {
        if (errEl) errEl.textContent = getApiBase()
          ? 'Invalid or expired token. Contact info@aielevate.xyz.'
          : 'Assessment API not configured yet. Token validation will work after Vercel deploy.';
        return;
      }
  ...
  function validateStep() {
    ...
    if (state.step === 0) {
      ...
      if (!org) return 'Enter the organization name to continue.';
      state.meta.organization = org;
      ...
      body = `
        ...
        <div class="edmp-assess-meta-grid">
          <label class="edmp-assess-field">
            ...
            <input id="edmpOrg" type="text" required placeholder="Company name" value="${escapeHtml(state.meta.organization)}" />
          </label>
          <label class="edmp-assess-field">
            <span>Contact name</span>
            <input id="edmpContact" type="text" placeholder="Assessment sponsor" value="${escapeHtml(state.meta.contact)}" />
          </label>
          <label class="edmp-assess-field">
            <span>Role / function</span>
            <input id="edmpRole" type="text" placeholder="CFO, Governance Lead..." value="${escapeHtml(state.meta.role)}" />
          </label>
        </div>
    ...
    mount.innerHTML = `
      <div class="edmp-assess-card edmp-report">
        ...
        <div class="edmp-report-hero">
          <div>
            <h2>${escapeHtml(report.meta.organization)}</h2>
        ...
        <section class="edmp-report-section edmp-consultant-edit">
          ...
          <label class="edmp-assess-field">
            ...
            <textarea id="edmpConsultantNotes" placeholder="Facilitator observations...">${escapeHtml(report.consultantNotes)}</textarea>
          </label>
        </section>
      </div>
    `;
